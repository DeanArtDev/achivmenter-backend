...
export default class FinancialReportRepository implements IFinancialReportRepository {
  ...
  public async create(report: FinancialReport): Promise<FinancialReportModel> {
  ...
  public async getAll(): Promise<FinancialReportModel[]> {
    return await this.db.client.financialReportModel.findMany({ include: { parts: true, period: true } });
  }
}
